...
class TestUserRegistrationLogic:
    ...
    def test_anonymous_cannot_register_non_latin_username(self, client: Client, registration_data: dict[str, str]):
        """Нельзя использовать не латинские буквы в юзернейме."""
        registration_data['username'] = 'Привет'
        old_count_users: int = User.objects.count()
